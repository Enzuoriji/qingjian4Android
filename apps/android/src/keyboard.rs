...
/// 得特意朝下瞄，快打时很容易滑歪。
///
/// **原先定的 12 点太近**：拇指快敲落地时会滚一下，随手就是十来点，于是正常打字时不时
/// 蹦出个角标符号（用户报的「快打误打符号」）。16 点取的是**两倍触摸阈值**
/// （[`TOUCH_SLOP`] 8 点，安卓自己也用 8dp）——UI 上「手势」与「手抖」的分界历来这么划。
///
/// **不能随便再往上加**：横向挪过**半个键宽**就落到隔壁键上了，而「离开按下的键 = 这一下作废」
/// 是另一条规矩（滑到别的键上要能反悔）。360 点宽的屏上键宽约 30 点，半键就是 15 点——
/// 阈值一过这条线，横向快敲就会**既不出符号、也不掉字**（手指出键 → 这一下作废）。
/// 键高 42 点给的是纵向的余量，横向没有。
///
/// 手感常数，只能靠真机调：嫌难滑就往下调，还误触就往上调一点。
pub(crate) const SWIPE: f32 = 16.0;
