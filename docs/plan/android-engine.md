...
**一、二两组 2026-09-22 做完了**（E1–E4）。三、四两组按下面这个顺序；E5 并进了 E7。
...
| **三** | 领域词库（E6） | 半天 | 待做 |
| **三** | **设置页 + 配置（E7）**——学习语言、震动开关与强度、云 API 的地址/模型/密钥、模糊音这些**都摆在这一页上** | 2.5 天 | 待做 |
| **四** | 云联想（E8） | 2 天 | 待做，**卡在 E7**：密钥得先有地方填 |
...
- **E8 云联想**（2 天）：接 `qingjian-predict`。桌面缺省就是关的，**安卓先不做不亏**；
  ...
  **安卓要补的**：
  ...
  4. **设置页那几格**（照 Windows 的「云服务」页）：开关、云端词格数 0–9、整句补全开关、
     接口地址、模型、密钥（密码框）、测试连接 + 状态行。**都摆在 E7 那一页上**。
