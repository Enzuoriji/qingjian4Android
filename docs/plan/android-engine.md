...
| 键盘 | 键盘高度、按键音… | 以后往下排 |
| 云 | **AI 模型 API**：开关、地址、模型、**密钥** | E8 卡在这一格 |
...
- **E8 云联想**（2 天）：接 `qingjian-predict`，要 API key、要网络权限、要隐私说明。
  桌面缺省就是关的，**安卓先不做不亏**。**但它是用户 2026-09-22 点名要的**
  （「后面也可以对接模型 api」），所以列进排期，只是排在 E7 后面——
  **密钥得先有地方填**，那一格在设置页上。
  接法待 E7 做完再细排：先看桌面那套（`qingjian-predict` 的 `PredictConfig` 与两壳的
  `attach_cloud`）能直接搬多少，安卓这边多出来的是**网络权限**与**密钥存哪**
  （桌面是环境变量 / 配置文件，安卓大概要走 Keystore 或至少是应用私有目录）。
