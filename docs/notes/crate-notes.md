...
字母页最下一行原是 9 个单位（逗号在空格右边、没有句号），比第 1、3 行的 10 个窄一整条，
**整排左右各缩进去半个键**；2026-09-20 补了一个 `KeyId::Period` 凑成 10 个才齐平。
同一天还改了两处**位置**：逗号挪到空格左边、`Mode` 挪到句号与回车之间——
最后是 `符(1.5) 123(1) ，(1) ｜空格(3)｜ 。(1) 中/英(1) 回车(1.5)`，
空格左右各 3.5 个单位，**正好居中**。两头的键 1.5、中间的 1，是为了让空格居中且回车不显窄。
这两条几何都有测试盯着（`layout.rs` 里那两条），别再凭感觉调。
字母键以外的键帽画的是**半角原字符**：全角与否由 `qingjian-core` 的标点表在打出去的时候转（`?`→`？`），
画死成固定全角在英文模式下就骗人了。**例外是 `Comma` 与 `Period`**——字母页底下这两个最常用，
键帽故意画全角（`，`/`。`）让人一眼认得，交出去的仍是半角 `,` / `.`，全角由引擎转。

安卓的候选条是 `renderer/bar/`（`render_bar`）：通栏一条，上排拼音直接复用候选窗的 `draw_top_line`，下排候选是新写的。
与候选窗最大的差别是**高度固定**——候选窗的宽高由内容量出来，候选条宽度是屏幕宽、高度由主题算死（`Renderer::bar_height`），
