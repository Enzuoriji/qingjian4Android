...
- **齿轮与候选条共用同一块地方**：`bar_height` 两态（组句 = 拼音行 + 候选行；没组句 = 30 点一条），
  `render_bar` 没组句时只画齿轮（`crate::gear::draw_gear`，与状态条同一个）+ 页码，
  组句时**不画齿轮**。所以「组句当中够不着剪贴板」是结构性的，不是忘了做。
- **页码写在齿轮那条上**（`Frame::footer`）：候选那对 `‹ ›` 只在组句时画，
