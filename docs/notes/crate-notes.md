...
**「松手清空」要落在气泡里（2026-09-23）**：`Keyboard::popup_rect()` 给出气泡**该在**的矩形
（底边贴 `rect.y`、横向对键心），Up 时拿它判「手指还在不在气泡上」。
**不能拿 `popup_origin()`**——那个位置在最左 / 最右的键上会被夹回屏幕（防出屏），
而手指停在键心正上方：测试里量到手指在 x=924、夹过的气泡只有 [666, 908]，
按它判的话 `⌫` 永远清不了。另外矩形要**趁 `presses` 还没动**取，
那之后 `pressed_key()` 就查不到了。

**触摸坐标自己换算，不用系统给的相对坐标（2026-09-23）**：壳里 `QingjianSurfaceView`
用 `getRawX/Y() − getLocationOnScreen()` 算出视图内坐标，**不用 `event.getX/Y()`**。
原因是输入法窗口的位置由系统摆，**它比窗口真正搬到位慢一拍**：候选条一长高
（没组句 → 组句中，差 45 点），窗口 42ms 就搬到新位置了，而输入系统过了 250ms 还在按
老位置换算——那一拍里 `event.getY()` 整整少算一个候选条的高度（模拟器上 90 像素 ≈ 一行键），
于是**新开一次组句之后紧跟着的那一下必串一行**（按 `n` 出 `j`，用户报的「打 en 不出 n」）。
`getRawY()` 是屏幕上真实的位置、`getLocationOnScreen()` 是视图此刻真正在哪儿，
两者之差与系统记的窗口位置无关。差值对所有手指同一个数，所以只算一次、各自加上。
（同一片代码上还修了两个更小的偏移：算触摸偏移的高度与画出来的位图**不是一个来源**
——`annotations` vs `bottom_line()`，开着云联想没挂释义表时差 40 像素；
以及那个高度两边**取整口径**不同，差 1 像素。两条都有回归测试盯着。）

**触摸的「送数据」与「收尾重画」分开（2026-09-23）**：壳那边 `QingjianSurfaceView.onTouch`
现在只把一根手指送进 Rust 并**返回掩码**，收尾（取位图、重画）交给新的 `onTouchDone`——
`ACTION_MOVE` 会按根逐次调前者，收尾只做一次。从前是每根手指各调一次带收尾的版本，
**两根手指移动就是整屏重画两遍**（改「逐根上报移动」那次引入的，同一天补上）。
掩码在视图那层按位或累积。
