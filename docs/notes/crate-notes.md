...
- **手势阈值 22 点，取角标只认往下滑**（`SWIPE`，2026-09-20）：⌫ 往上滑够这么远 = 松手清空。
  中间走过一段弯路：12 点（太近，快打误蹦符号）→ 16 点且**四个方向都算**（用户 2026-09-20 要的，
  说只认往下得特意朝下瞄、快打容易滑歪）→ **仍然误触，收回成一个方向**并提到 22 点。
  收回的理由是**横向结构上救不了**：「手指出了键 = 这一下作废」与阈值线是**重合**的
  （都卡在半个键宽，约 15 点），往上加只会变成「符号不出、字母也一起丢」。
  只留往下之后两件事同时成立：误触面砍到四分之一；阈值改跟**键高**比（半步 21~27 点），
  才有余量从 16 提到 22。代价是另外三个方向的角标只能去 `123` / 符 面板打（那些字符都在）。
  手感常数，只能靠真机调；测试见 `session/tests.rs::the_hint_only_comes_out_when_you_swipe_down`。
  **测试里别正好用阈值那个数**：位移是 `(y + d) - y` 算的，f32 下未必正好等于 `d`，
  卡在阈值上会变成浮点运气（实测两个键一个过一个不过）——用 `swipe_past()`。
...
- **候选条横滑 = 滚**（2026-09-21；这天先做过一版「横滑选词」，用户说理解错了，换掉了）。
  手指在候选条上横着拖，那条带子**跟着手指平移**（`Session::scroll_by`，按**位移增量**加，
  所以是跟手的），滚过一页的宽度就翻下一页、往回滚过页首就翻回上一页（从那一页开头接着，
  手指多拖的那点吸收掉——上一页多宽这边算不出来）。
  页宽从**上一帧的命中矩形**量（`page_width`：最后那格右边缘 + 回滚过的量），差一帧，
  手指每动一下都重画，追得上。**量页宽要在加上这次的位移之前**——踩过：先加再量，
  量出来的多算了这一段，永远够不到翻页的线。
  **拖动只是看，不上屏**：要选还得点一下（或空格上屏本页第一个）。参考项目 fcitx5-android
  的候选条**根本不滚**（`canScrollHorizontally=false`，满 `maxSpanCount` 就收、多的进展开面板），
  所以这条是照「看得见更多候选」的目标自己定的，不是抄来的。
- **高亮不跟着滚动走**：`Session::highlight`（页内下标）只在重查候选与翻页时归零，
  空格上屏与「组句中打标点先上屏高亮候选」都走 `highlighted_candidate`，**「高亮在哪」只有一个说法**。
...
- **候选条一页画几个，按这次打了几个音节定**（2026-09-20 改，原先写死 5；桌面 9 个）：
  1 个音 6 个、2~3 个音 5 个、4 个音以上 4 个。打的音越多候选越长、一格越占地方，所以音少时反而多给一两个。
  **页大小是个定值**（音数在整段组句里不变），所以「第几页从第几条起」仍是 `page * 页大小` 一个乘法——
  翻页与页码都不用改。这也是没走「按宽度塞」那条路的原因：那个得把全部候选量一遍才知道页数。
  手感常数在 `session/mod.rs` 的 `page_size_for`。
