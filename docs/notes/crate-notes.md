...
- **随包资源目录（`filesDir/bundle/`）**：壳把 APK 的 assets 解到这儿，整个目录交给 Rust 当 `bundle`。
  里头现在有三样：emoji 字体与两张 emoji 表、表情面板两张表（见 `assets/emoji/README.md`）、
  以及英文词表 `english.tsv`。**有哪张用哪张**：emoji 那几张缺一张键盘就画不出表情，
  所以壳那边是「有一个解不出来就整个放弃」（`ensureExtras`）；**英文词表是可选的**，
  单独解、失败不拦——少了它英文模式退回直输，不该把 emoji 一起拖下水。
  Rust 侧读不到哪张就少哪块功能，都不影响启动。这个目录原来叫 `emoji`，2026-09-22 加英文词表时改的名。
  词库 `dict.qj` **不在这里**：它跟别的产品数据一样单独解到 `filesDir` 根上，路径由壳显式传给 `open`。
