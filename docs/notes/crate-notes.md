...
下排每格宽度**按内容分**（2026-09-20 改，原先均分）：先给每格一份底线（`MIN_CELL_WIDTH` 48 点，
不然单字候选那格细得点不着），剩下的按**各自超出底线多少**分——长的多分。一页里的词全装得下时
一格都不会截。格间留一条缝，词实在装不下才截断补省略号；
`×` / `‹` / `›` 与每个候选的位置一并返回，供命中测试。
...
- **横滑选词**（2026-09-21 换掉原先的「横滑翻页」）：手指按在候选条上横着滑，
  **高亮跟着手指走**（`Session::slide_highlight`，按手指 x 落在哪个命中矩形里算），
  **松手就把高亮那个上屏**；滑过这一页的头尾会**接着翻页**（往后翻落在新页第一格、
  往前翻落在最后一格）——不这样，滑到页边界就断了。
  高亮的下标存在 `Session::highlight`（页内下标），空格上屏与「组句中打标点先上屏高亮候选」
  走的都是它（`highlighted_candidate`），**「高亮在哪」只有这一个说法**。
  翻页本身仍然只有 `‹ ›` 按钮这条路（外加滑过头时的自动翻），它也是**只看不选**的正路。
- **拖出候选条 = 这一下作废**（`BarPress::outside`）。上面那条是「松手就上屏」，
  得有地方退：手指往上顶出屏幕、往下溜进键盘，都算拖出去了。与键盘上「滑出键外作废」一个规矩。
